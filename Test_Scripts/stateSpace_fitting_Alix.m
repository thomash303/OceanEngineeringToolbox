...
% Impluse reponse System identtification is numerically difficult
%Lets convert impulse response to Unit step response
% Unit step response=(running integral of impulse response) * step time
% Unit step input is simply a unit amplitude dc signal
dt=t1(2)-t1(1);          % Extract sample time
u1_op=cumsum(k1(:))*dt;  % Unit step response from impluse response
u1_ip=ones(size(u1_op));  % Unit Step input, simply a dc signal of unit amplitude

Data = iddata(u1_op, u1_ip,dt); % unit step response input and output passed
                                % to iddata
