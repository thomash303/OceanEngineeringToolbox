within OceanEngineeringToolbox.Multibody.Joints;


model Revolute
  "Revolute joint (1 rotational degree-of-freedom, 2 potential states, optional axis flange). Code directly imported from the MSL in the case that there are issues importing Modelica MBS models." 
  import Modelica.Mechanics.MultiBody.Interfaces.PartialElementaryJoint;
  import Modelica.Units.SI;
  import Modelica.Mechanics.Rotational.{Interfaces, Components, Sources};
  ...
  Interfaces.Flange_b support if useAxisFlange
  ...
  input Types.SpecularCoefficient
...
protected
  ...
  parameter Real e[3](each final unit="1")=Vectors.normalizeWithAssert(n)
...
protected
  ...
  Interfaces.InternalSupport internalAxis(tau=tau)
    annotation (Placement(transformation(extent={{-10,90},{10,70}})));
  Sources.ConstantTorque constantTorque(tau_constant=0) if not useAxisFlange
