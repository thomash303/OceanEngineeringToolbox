model IMC_DOL_steady_load "Test example: InductionMachineSquirrelCage direct-on-line"
  ...
  Modelica.Mechanics.Rotational.Components.Inertia loadInertia(J = 0.5) annotation(
    Placement(transformation(origin = {-6, 34}, extent = {{40, -50}, {60, -30}})));
...
equation
  ...
  connect(loadInertia.flange_a, aimc.flange) annotation(
    Line(points = {{34, -6}, {0, -6}, {0, -40}}));
  connect(damper.flange_a, aimc.flange) annotation(
    Line(points = {{34, -70}, {0, -70}, {0, -40}}));
  connect(damper.flange_b, fixed.flange) annotation(
    Line(points = {{54, -70}, {58, -70}, {58, -74}}));
  annotation(
