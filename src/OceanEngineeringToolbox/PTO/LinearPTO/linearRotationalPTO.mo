...
model linearRotationalPTO
  ...
  parameter SI.RotationalSpringConstant Kpto = 0 "PTO stiffness coefficient" annotation(
    Dialog(group = "PTO Coefficients"));
  parameter SI.RotationalDampingConstant Cpto = 0 "PTO damping coefficient" annotation(
    Dialog(group = "PTO Coefficients"));
  parameter Types.Axis n = {0, 0, 1} "Axis of rotation resolved in frame_a (= same as in frame_b)" annotation(
    Dialog(group = "Joint Parameters"));
  parameter SI.Angle s_ref = 0 "Reference or equilibirum position (Unstretched spring angle)" annotation(
    Dialog(group = "Joint Parameters"));
  parameter SI.Position phi_start = 0 "Initial relative rotation angle from frame_a to frame_b" annotation(
    Dialog(group = "Initialization"));
  parameter SI.Position w_start = 0 "First derivative of phi_start (relative angular velocity)" annotation(
    Dialog(group = "Initialization"));
  parameter SI.Position a_start = 0 "Second derivative of s_start (relative angular acceleration)" annotation(
    Dialog(group = "Initialization"));
  SI.Power P "PTO power output";
  
  Joints.Revolute revolute(animation = false, n = n, useAxisFlange = true, phi(start = phi_start, fixed = true), w(start = w_start, fixed = true), a(start = a_start, fixed = true))  annotation(
