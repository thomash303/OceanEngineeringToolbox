...
model HydrodynamicBody "Model containing the hydrodynamic body used to represent the dynamics floating body. Instantiated directly in the top-level of the model."
  ...
  inner parameter Integer bodyIndexTemp = bodyIndex "Body index used for the radiation force (to avoid instantiation issue with replaceable objects)" annotation(
    HideResult = true,
    Dialog(enable = false, tab = "Misc"));
  // Body index used for the radiation force (to avoid instantiation issue with replaceable objects)
  inner parameter Integer nMETemp = nME "Number of Morison Morison elements (to avoid instantiation issue with replaceable objects)" annotation(
    HideResult = true,
    Dialog(enable = false, tab = "Misc"));
  // Mass parameters
  ...
  replaceable Forces.Radiation radiation(file = fileDirectory.file) if enableRadiationForce constrainedby Forces.Radiation annotation(
     Dialog(group = "Radiation"),
  ...
  // Drag coefficients
  parameter Real Cv[6](each min = 0) = {0, 0, 0, 0, 0, 0} "Linear damping coefficient vector" annotation(
    HideResult = true,
    Dialog(group = "Damping/Drag", enable = enableDampingDragForce));
  parameter Real CD[6](each min = 0) = {0, 0, 0, 0, 0, 0} "Quadratic drag coefficient vector" annotation(
    HideResult = true,
    Dialog(group = "Damping/Drag", enable = enableDampingDragForce));
  parameter Real Ad[6](each min = 0) = {0, 0, 0, 0, 0, 0} "Characteristic area vector" annotation(
    HideResult = true,
    Dialog(group = "Damping/Drag", enable = enableDampingDragForce));
  // Morison
  ...
  replaceable Forces.Morison morison(nME = nME, rME = rME, nHatME = nHatME, Cfk = Cfk, Cd = Cd, Ac = Ac, Cam = Cam, VME = VME) if enableMorisonForce "Select the desired wave and current and wave kinematic model" annotation(
    Placement(transformation(origin = {-4, 88}, extent = {{-14, -14}, {14, 14}})),
    choices(checkBox = true),
    Dialog(group = "Morison"));
  // Morison parameters
