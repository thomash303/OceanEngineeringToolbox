...
model HydrodynamicBody
  ...
  // Radiation
    Forces.Radiation radiation(filePath = fileDirectory.filePath, hydroCoeffFile = fileDirectory.hydroCoeffFile, bodyIndex = bodyIndex, enableB2B = enableB2B) if enableRadiationForce annotation(
  ...
  parameter Boolean enableB2B = false "Switch to enable/disable the radiation force calculation including body-to-body interactions" annotation(
    choices(checkBox = true),
    Dialog(group = "Radiation"));
  // Hydrostatic
