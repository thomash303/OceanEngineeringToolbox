...
model ExcitationNLFK
  ...
  // Enabling
  parameter Boolean meanDriftEnable = false "Switch to enable/disable the mean drift force calculation";
 ...
 SubForces.ExcitationForces.MeanDriftForce meanDriftForce(zeta = environment.wave.zeta, n_omega = environment.wave.n_omega, omega = environment.wave.omega, phi = environment.wave.phi, ramp = environment.wave.ramp, Trmp = environment.Trmp, file = file, bodyIndex = bodyIndex, waveHeading = environment.wave.waveHeading, waveHeadingSpreadBins = environment.wave.waveHeadingSpreadBins, spreadBinCentres = environment.wave.spreadBinCentres) if meanDriftEnable annotation(
...
equation
 ...
 connect(meanDriftForce.frame_a, frame_a) annotation(
    Line(points = {{-10, 34}, {-100, 34}, {-100, 0}}, color = {95, 95, 95}));
 annotation(
    Diagram);
end ExcitationNLFK;
