...
model BodyMassB2B "Model containing the modified MSL mass"
  ...
  outer Real aCoupled[nDoF] "Vector containing translational accelerations of all hydrodynamic bodies";
  outer Real alphaCoupled[nDoF] "Vector containing translational accelerations of all hydrodynamic bodies";
  ...
  Real vCheck[3];
protected
  SI.Acceleration acceleration[6] = cat(1, a, z_a);
...
// translational kinematic differential equations
  ...
  /* Newton/Euler equations with respect to center of mass
      ...
      */
  // Shared acceleration vector for B2B interactions
  for i in 1:bodyDoF loop
    aCoupled[bodyDoF*(bodyIndex - 1) + i] = a[i];
    alphaCoupled[bodyDoF*(bodyIndex - 1) + i] = z_a[i];
  end for;     

  // Note a now defined as local translational acceleration
  f_element = m*a + Ainf11*aCoupled + Ainf12*alphaCoupled;
  t_element = I*z_a + cross(w_a, I*w_a) + Ainf21*aCoupled + Ainf22*alphaCoupled;
  frame_a.f = f_element;
  frame_a.t = t_element;
  annotation(
