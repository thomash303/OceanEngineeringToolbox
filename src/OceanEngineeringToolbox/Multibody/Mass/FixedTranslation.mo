...
model FixedTranslation "Fixed translation of frame_b with respect to frame_a. Code directly imported from the MSL in the case that there are issues importing Modelica MBS models."
  
  // Importing from the MSL
  import Modelica.Units.Conversions.to_unit1;
  import Modelica.Units.SI;
  import Modelica.Mechanics.MultiBody.{Types, Visualizers, Frames, World, Interfaces};
  ...
  parameter Types.Axis widthDirection={0,1,0}
    "Vector in width direction of shape, resolved in frame_a" annotation (
      ...
      group="if animation = true",
      enable=animation));
  parameter SI.Length length=Vectors.length(r - r_shape)
  ...
  parameter Types.ShapeExtra extra=0.0
    ...
    annotation (Dialog(
      ...
      group="if animation = true",
      enable=animation));
  input Types.Color color=Types.Defaults.RodColor
