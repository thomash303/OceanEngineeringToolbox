within OceanEngineeringToolbox.Multibody.Worlds;
...
model World
  ...
  import Modelica.Mechanics.MultiBody.{Types, Visualizers, Frames, Forces, Interfaces};
  ...
  parameter SI.Acceleration g=g_n "Constant gravity acceleration"
    annotation (Dialog(enable=gravityType == Types.GravityTypes.UniformGravity));
  parameter Types.Axis n={0,-1,0}
    "Direction of gravity resolved in world frame (gravity = g*n/length(n))"
    annotation (Evaluate=true, Dialog(enable=gravityType == Types.GravityTypes.UniformGravity));
  parameter Real mu(
    unit="m3/s2",
  ...
  parameter SI.Position gravityArrowTail[3]={0,0,0}
    ...
    annotation (Dialog(tab="Animation", group=
          ...
          enable=enableAnimation and animateGravity and gravityType == Types.GravityTypes.UniformGravity));
  ...
  parameter SI.Diameter gravitySphereDiameter=12742000
    ...
    annotation (Dialog(tab="Animation", group=
          ...
          enable=enableAnimation and animateGravity and gravityType == Types.GravityTypes.PointGravity));
  input Types.Color gravitySphereColor={0,230,0} "Color of gravity sphere"
    annotation (Dialog(colorSelector=true, tab="Animation", group=
          "if animateGravity = true and gravityType = PointGravity",
          enable=enableAnimation and animateGravity and gravityType == Types.GravityTypes.PointGravity));

  parameter Types.Axis groundAxis_u=if abs(n[1])>=0.99 then {0,1,0} else {1,0,0}
    ...
    annotation (Dialog(
      ...
      enable=enableAnimation and animateGround and gravityType == Types.GravityTypes.UniformGravity));
  ...
  replaceable function gravityAcceleration =
       Forces.Internal.standardGravityAcceleration (
           gravityType=gravityType, g=g*Vectors.normalizeWithAssert(n), mu=mu)
       constrainedby Interfaces.partialGravityAcceleration
...
protected
  ...
  Visualizers.Advanced.Shape gravityArrowLine(
    shapeType="cylinder",
    ...
    specularCoefficient=0) if enableAnimation and animateGravity and gravityType == Types.GravityTypes.UniformGravity;
  Visualizers.Advanced.Shape gravityArrowHead(
    shapeType="cone",
    ...
    widthDirection={0,1,0},
    color=gravityArrowColor,
    r_shape=gravityArrowTail + Vectors.normalize(
                                                n)*gravityLineLength,
    specularCoefficient=0) if enableAnimation and animateGravity and gravityType == Types.GravityTypes.UniformGravity;

  // Point gravity visualization
  parameter Integer ndim_pointGravity=if enableAnimation and animateGravity
       and gravityType == Types.GravityTypes.UniformGravity then 1 else 0;
  Visualizers.Advanced.Shape gravitySphere(
    shapeType="sphere",
    ...
    specularCoefficient=0) if enableAnimation and animateGravity and gravityType == Types.GravityTypes.PointGravity;
...
*/
  ...
  Visualizers.Advanced.Surface surface(
    final multiColoredSurface=false,
    final wireframe=false,
    final color=groundColor,
    final specularCoefficient=0,
    final transparency=0,
    final R=Frames.absoluteRotation(
      Frames.from_nxy(n, groundAxis_u),
      Frames.axesRotations({1,2,3}, {pi/2,pi/2,0}, {0,0,0})),
    final r_0=zeros(3),
    final nu=2,
    final nv=2,
    redeclare function surfaceCharacteristic =
      Visualizers.Advanced.SurfaceCharacteristics.rectangle(
      lu=groundLength_u, lv=groundLength_v)) if
      enableAnimation and animateGround and gravityType == Types.GravityTypes.UniformGravity;
equation
  Connections.root(frame_b.R);

  assert(Vectors.length(n) > 1e-10,
