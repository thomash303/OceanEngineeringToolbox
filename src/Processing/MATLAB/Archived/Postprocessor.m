%% Ocean Engineering Toolbox - Post-Processing Module
%{
A post-processing file for the OET to post-process Modelica simulation data. 
The user must specify the result folder, result file, the number of bodies 
in the simulation, and the instantiated hydrodynamicBody Modelica model 
names (cap sensitive).
base path of the BEM output data and the desired name of the output .mat
file.
%}
...
% % fileName = fullfile('Result_File','File_Name_res.csv');  % Path to current file                  
...
%% Wave Data
% % User Input (only 0 and 1 wave inputs are currently supported)
% waves = 1;
% 
% 
% % Wave Parameters
...
%% Mooring Data
% User Input (preprocessor currently only supports 0 or 1 moorings)
moorings = 1;
mooringName = {'linearMooringForce'};
...
%% Save
save('deviceName_Output.mat','body', 'wave','mooring','pto')
