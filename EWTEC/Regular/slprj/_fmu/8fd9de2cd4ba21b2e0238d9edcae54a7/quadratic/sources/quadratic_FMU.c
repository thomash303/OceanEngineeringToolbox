...
#define MODEL_IDENTIFIER quadratic
#define MODEL_GUID "{fcd91194-d5f8-425b-bc46-6251cbc4a366}"
...
#define FMI2_FUNCTION_PREFIX quadratic_
...
extern void quadratic_setupDataStruc(DATA *data, threadData_t *threadData);
#define fmu2_model_interface_setupDataStruc quadratic_setupDataStruc
...
void setDefaultStartValues(ModelInstance *comp) {
  ...
  comp->fmuData->modelData->realParameterData[0].attribute.start = 2.0;
}
// Set values for all variables that define a start value
OMC_DISABLE_OPT
void setStartValues(ModelInstance *comp) {
...
fmi2Status setReal(ModelInstance* comp, const fmi2ValueReference vr, const fmi2Real value) {
  ...
  if (vr < 2 && (comp->state == model_state_instantiated || comp->state == model_state_initialization_mode)) {
